...
class JanelaInserirVinyl:
    ...
    def registar_vinyl(self):
        ...
        # Verificar se o título do vinyl já existe na tabela
        cursor.execute("SELECT * FROM vinyl WHERE titulo=?", (titulo_vinyl,))
        vinyl_existente = cursor.fetchone()

        if vinyl_existente:
            # Se o vinyl já existir, exibir mensagem e não realizar a inserção novamente
            self.mensagem_registo_concluido = Label(self.janela_inserir_vinyl, text="Erro, este vinyl já está registado.", fg='red')
            ...
            # Inserir o vinyl apenas se não existir na tabela
            cursor.execute("INSERT INTO vinyl (titulo, artista, editora, ano, genero) VALUES(?,?,?,?,?)",
                        (titulo_vinyl, artista_vinyl, editora_vinyl, ano_vinyl, genero_vinyl)) # falta o valor para a imagem
            conn.commit()
            conn.close()
